...
module.exports = {
    Query: {
        ...
        me: (_, __, {dataSources, user}) => {
            return user //dataSources.userAPI.findOrCreateUser()
        }
    },
    Mutation: {
        login: async (_, {email}, {dataSources, user}) => {
            const loggedUser = await dataSources.userAPI.findOrCreateUser({email})
            
            if(loggedUser){
                loggedUser.token = Buffer.from(email).toString('base64')
                return loggedUser
            }
        },
        ...
        cancelTrip: async (_, {launchId}, {dataSources}) => {
            const result = await dataSources.launchAPI.getLaunchById({launchId})
